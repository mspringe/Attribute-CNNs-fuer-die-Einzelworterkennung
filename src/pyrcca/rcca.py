...
def kcca(data, reg=0., numCC=None, kernelcca=True, ktype='linear',
    ...
    # number of data-sets
    nDs = len(kernel)
    # numbers of features in datasets
    nFs = [k.shape[0] for k in kernel]
    # number of relevant canonical components
    ...
    maxCC = LH.shape[0]
    # r: eigenvalues, Vs: eigenvectors
    r, Vs = eigh(LH, RH, eigvals=(maxCC - numCC, maxCC - 1))
    r[np.isnan(r)] = 0
    # indices of eigenvalues descending
    rindex = np.argsort(r)[::-1]
    comp = []
    # eigenvectors sorted by descending eigenvalues
    Vs = Vs[:, rindex]
    for i in range(nDs):
        # eigenvectors of dataset up to the number of relevant canonical components
